...
alter table public.profiles
  add constraint cv_import_shape check (
    cv_import is null or (
      coalesce(jsonb_typeof(cv_import->'filename'), '') = 'string'
      and coalesce(jsonb_typeof(cv_import->'imported_at'), '') = 'string'
      and coalesce(jsonb_typeof(cv_import->'source'), '') = 'string'
      and coalesce(jsonb_typeof(cv_import->'locale'), '') = 'string'
      and (cv_import->>'imported_at')::timestamptz is not null
    )
  );

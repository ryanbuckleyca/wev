-- Bilingual public org copy: description and mission in en + fr.
-- Legacy description / mission_statement columns are retained and synced by
-- application writers (COALESCE(en, fr)) for search/RPC compatibility.
-- SSE reasoning bilingual keys live in sse_details JSON (reasoning_en / reasoning_fr).

ALTER TABLE public.organizations
  ADD COLUMN IF NOT EXISTS description_en text,
  ADD COLUMN IF NOT EXISTS description_fr text,
  ADD COLUMN IF NOT EXISTS mission_statement_en text,
  ADD COLUMN IF NOT EXISTS mission_statement_fr text;

COMMENT ON COLUMN public.organizations.description_en IS
  'Public organization description in English.';
COMMENT ON COLUMN public.organizations.description_fr IS
  'Public organization description in French.';
COMMENT ON COLUMN public.organizations.mission_statement_en IS
  'Public organization mission statement in English.';
COMMENT ON COLUMN public.organizations.mission_statement_fr IS
  'Public organization mission statement in French.';

-- Seed English columns from existing monolingual fields (current content is mostly EN).
...
UPDATE public.organizations
SET mission_statement_en = mission_statement
WHERE mission_statement_en IS NULL
  AND mission_statement IS NOT NULL
  AND btrim(mission_statement) <> '';

-- Promote existing sse_details.reasoning into reasoning_en when missing.
UPDATE public.organizations
SET sse_details = sse_details || jsonb_build_object(
  'reasoning_en', sse_details->>'reasoning'
)
WHERE sse_details IS NOT NULL
  AND sse_details ? 'reasoning'
  AND COALESCE(btrim(sse_details->>'reasoning'), '') <> ''
  AND (
    NOT (sse_details ? 'reasoning_en')
    OR COALESCE(btrim(sse_details->>'reasoning_en'), '') = ''
  );
...
COMMENT ON FUNCTION public.get_active_organizations(timestamp with time zone, integer, integer, text, boolean, text[], text[], text[], uuid, text) IS
  ...
  'Includes bilingual description/mission columns. Date filter via try_parse_job_date_posted(). '
  'p_user_id is ignored; auth.uid() is used for profile access.';

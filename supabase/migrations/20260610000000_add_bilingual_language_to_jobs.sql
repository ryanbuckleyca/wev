...
-- Backfill existing data: ensure all Ma Communauté jobs are 'fr'
-- (Using source name to be environment-agnostic instead of hardcoding UUIDs)
UPDATE jobs
SET language = 'fr'
WHERE source_id IN (
    SELECT id FROM sources WHERE name ILIKE '%ma communauté%' OR name ILIKE '%macommunaute%'
);

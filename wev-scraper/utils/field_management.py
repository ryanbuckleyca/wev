...
def build_update_row_with_field_preservation(job_row: dict, source_id: int, existing_data: dict) -> dict:
    ...
    for _group_name, config in field_groups.items():
        should_process = is_truthy_env(config["process_flag"])
        should_reprocess = is_truthy_env(config["reprocess_flag"])
...
def print_field_groups_help():
    ...
    print("\n📋 Control Logic:")
    print("   Process Flag = 0 → Never process, always preserve existing")
    print("   Process Flag = 1 + Reprocess Flag = 0 → Process but preserve existing")
    print("   Process Flag = 1 + Reprocess Flag = 1 → Process and update existing")

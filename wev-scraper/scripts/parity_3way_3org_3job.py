#!/usr/bin/env python
"""3-way dry-run parity: prod | gemini+tav | groq+tav.

Forces Tavily grounding for every re-run and isolates each provider.
Never writes to Supabase. Never commits. No Cerebras.

Usage:
  CONFIRM_PROD_RUN=YES ./venv/bin/python scripts/parity_3way_3org_3job.py --prod
"""
...
LOG_PATH = Path("/tmp/parity_3way_3org_3job.log")
OUT_JSON = Path(__file__).resolve().parent / "parity_3way_3org_3job.json"
OUT_MD = Path(__file__).resolve().parent / "parity_3way_3org_3job.md"
...
logger = logging.getLogger("parity_3way")

# Fresh sample — not used in prior parity / tavily_job_parity / 4-way runs
# Orgs: Green Communities Canada (SSE NP), Rainbow Songs (for-profit no),
# City of Markham (gov no)
ORG_IDS = [1419, 1416, 1413]
JOB_IDS = [
    "a47de111-4581-4f7d-b752-4464e004f469",  # Le Dépôt — strong_yes
    "7483d32e-7a8f-4f89-a06d-dac3951dd8e8",  # Agence Ometz — weak_yes
    "6463983f-45fd-4e8b-95ec-ab9987c52289",  # Cité Construction — no
]
...
def write_md(summary: dict) -> None:
    lines: list[str] = []
    lines.append("# 3-way parity: prod | gemini+tav | groq+tav")
    lines.append("")
    lines.append(f"Generated: `{summary['generated_at']}`")
    lines.append("")
    lines.append(
        "Setup: Tavily-always (`FORCE_GROUNDING=1`, `USE_GOOGLE_SEARCH_GROUNDING=0`); "
        "Gemini-only then Groq-only; Ollama skipped; no Cerebras; no prod writes."
    )
    lines.append("")

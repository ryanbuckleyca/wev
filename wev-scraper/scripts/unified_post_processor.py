...
def process_jobs_unified(opts: ProcessingOptions | None = None) -> Dict[str, Any]:
    ...
    # Process in batches
    batch_size = 10
    max_batches = 10 if opts.dry_run else (len(all_eligible) + batch_size - 1) // batch_size
    processed_count = 0
    result: dict = {}
    for batch_idx, i in enumerate(range(0, len(all_eligible), batch_size)):
        ...
        for job, job_result in zip(batch, results_list, strict=True):
            ...
            if opts.dry_run and opts.task in ("sse", "all") and "is_sse" in job_result:
                old_val = job.get("is_sse")
                new_val = job_result["is_sse"]
                delta = "CHANGED" if old_val != new_val else "unchanged"
                title = job.get("job_title", "?")
                org = job.get("organization", "?")
                reasoning = job_result.get("sse_reasoning") or job_result.get("sse_details") or ""
                print(f"  [{delta}] {job['id'][:8]}... {title} @ {org}")
                print(f"         was: {old_val}  →  would be: {new_val}")
                if reasoning:
                    print(f"         why: {reasoning[:300]}")
    ...
    if task in ["all", "sse"] and "is_sse" in job_result:
        ...
        if "sse_confidence" in job_result or "sse_details" in job_result:
            update_data["sse_details"] = json.dumps({
                ...
                "reasoning": job_result.get("sse_reasoning") or job_result.get("sse_details", "Generated by unified processor")
            })

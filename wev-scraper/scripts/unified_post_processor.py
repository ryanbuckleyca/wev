...
Usage:
    python unified_post_processor.py [--task sse|values|summary|all] [--page-limit N]
...
def process_jobs_unified(opts: ProcessingOptions | None = None) -> Dict[str, Any]:
    ...
    for i in range(0, len(all_eligible), batch_size):
        ...
        for job, job_result in zip(batch, results_list, strict=True):
            ...
            if not opts.dry_run:
                update_data = _build_update_data(opts.task, job_result, job, result.get("provider"))
                if update_data:
                    try:
                        _try_db_write(job, update_data, supabase)
                        if _touches_match_relevant(update_data):
                            _enqueue_job_match_recalc(job["id"], supabase)
...
def _build_update_data(task: TaskType, job_result: dict, job: dict | None = None, provider: str | None = None) -> dict:
    ...
    Job SSE is gated on the linked organization's ``is_sse``: a role cannot
    be SSE unless the employer is. See ``utils.job_sse``.
    """
    ...
    if task in ["all", "sse"] and "is_sse" in job_result:
        ...
        if not isinstance(proposed, bool):
            proposed = bool(proposed) if proposed is not None else None
        resolved = resolve_job_is_sse(proposed, org_is_sse_from_job_row(job))
        # Defer writing when the org is not yet assessed and the model said yes —
        # leave is_sse null so catch-up retries after org assessment.
        if not job_sse_was_deferred(proposed, resolved):
            update_data["is_sse"] = resolved
            gated = job_sse_was_gated(proposed, resolved)
            flags = annotate_sse_details_flags(
                list(job_result.get("flags") or []),
                gated=gated,
            )
            if provider:
            ...
            sse_details = {
                ...
                "reasoning": gated_sse_reasoning(
                    job_result.get("sse_details", "Generated by unified processor"),
                    gated=gated,
                ),
                "must_haves_met": job_result.get("must_haves_met", []),
                "nice_to_haves_met": job_result.get("nice_to_haves_met", []),
                "flags": flags,
                "classified_at": datetime.now(timezone.utc).isoformat(),
                "reviewed": False,
            }
            update_data["sse_details"] = json.dumps(sse_details)
...
_MATCH_RELEVANT_KEYS = frozenset({
    "values",
    "values_rated",
    "skills",
    "work_type",
...
def _touches_match_relevant(update_data: dict) -> bool:
    """Return True if the update payload contains a field that affects scoring."""
    return any(k in _MATCH_RELEVANT_KEYS for k in update_data)
...
def is_transient_db_error(e: Exception) -> bool:
    ...
    if code:
        ...
        if code_str.startswith("53") or code_str.startswith("08"):
            return True
        # 57014 = statement_timeout: often transient when a trigger/adjacent work
        # takes too long under load; retry the row mutation so a slow recalc does
        # not permanently lose scraper writes.
        if code_str in {"57014", "40P01", "502", "503", "504"}:
            return True
...
def _enqueue_job_match_recalc(job_id: str, db_client) -> None:
    """Enqueue async match recalculation.

    The trigger also does this on score-relevant UPDATEs, but having the scraper
    enqueue explicitly keeps correctness on older DBs before the migration lands
    and makes the scraper the clear owner of the workflow.
    """
    try:
        db_client.rpc("enqueue_job_match_recalc", {"p_job_id": job_id}).execute()
    except Exception as e:
        scraper_log(
            f"  ! Could not enqueue job match recalc for {job_id[:8]} (non-fatal, "
            f"trigger/cron will back-fill): {e}"
        )

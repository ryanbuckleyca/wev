#!/usr/bin/env python
"""3-way dry-run parity v2: prod | gemini+tav | groq+tav.

Post-fix retest with NEW orgs/jobs (excludes prior parity IDs).
Forces Tavily grounding for every re-run and isolates each provider.
Never writes to Supabase. Never commits. No Cerebras.

Usage:
  CONFIRM_PROD_RUN=YES ./venv/bin/python scripts/parity_3way_3org_3job_v2.py --prod
"""
...
LOG_PATH = Path("/tmp/parity_3way_3org_3job_v2.log")
OUT_JSON = Path(__file__).resolve().parent / "parity_3way_3org_3job_v2.json"
OUT_MD = Path(__file__).resolve().parent / "parity_3way_3org_3job_v2.md"
...
logger = logging.getLogger("parity_3way_v2")
...
def write_md(summary: dict) -> None:
    lines: list[str] = []
    lines.append("# 3-way parity v2: prod | gemini+tav | groq+tav")
    lines.append("")
    lines.append(f"Generated: `{summary['generated_at']}`")
    lines.append("")
    lines.append(
        "Setup: post-fix retest; Tavily-always (`FORCE_GROUNDING=1`, "
        "`USE_GOOGLE_SEARCH_GROUNDING=0`); Gemini-only then Groq-only; "
        "Ollama skipped; no Cerebras; no prod writes. New IDs (excludes prior parity)."
    )
    lines.append("")

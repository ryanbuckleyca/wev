...
export function createDisplayKey(
  option: Pick<Option, 'label'> & { description?: string; definition?: string; scopeNote?: string },
): string {
      ...
      <div className="relative">
        ...
        >
          <div className="relative">
            ...
            />
            {loading && (
              <span className="absolute right-3 top-1/2 -translate-y-1/2 z-30 pointer-events-none">
                <LoadingIndicator
                  size="sm"
                  fullScreen={false}
                  message={null}
          ...
          {open && (
            <Command.List
              className={`absolute left-0 right-0 z-20 max-h-80 overflow-y-auto rounded-lg border border-[var(--border)] bg-[var(--background)] shadow-lg ${

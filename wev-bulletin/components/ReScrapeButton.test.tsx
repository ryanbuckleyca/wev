...
const triggerOk = () => Promise.resolve({ ok: true, json: () => Promise.resolve({}) } as Response);
...
describe('ReScrapeButton', () => {
  ...
  it('shows error and resets on workflow failure conclusion', async () => {
    ...
    global.fetch = vi.fn().mockReturnValueOnce(triggerOk()).mockReturnValueOnce(pollFailure());
  ...
  it('does not throw or call setState after unmount', async () => {
    global.fetch = vi.fn().mockReturnValueOnce(triggerOk()).mockReturnValueOnce(pollRunning());
  ...
  it('clears localStorage on successful completion', async () => {
    global.fetch = vi.fn().mockReturnValueOnce(triggerOk()).mockReturnValueOnce(pollSuccess());

  ...
  return (
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        <p
          className={`text-[13px] font-semibold truncate ${isRanked ? 'text-gray-800 dark:text-zinc-100' : 'text-gray-500 dark:text-zinc-400'}`}
        >
          {item.label}
        </p>
        {item.sublabel && (
...
function SortableDivider({
  ...
  return (
    <div
      ref={setNodeRef}
      style={style}
      className="flex items-center gap-2 py-1 select-none pointer-events-none"
    >
...
function StaticEmptyZone({ text }: { text: string }) {
  ...
  return (
    <div className="flex items-center justify-center rounded-lg border-2 border-dashed py-4 border-gray-200 bg-gray-50/30 dark:border-zinc-700 dark:bg-zinc-900/20">
      <span className="text-[11px] font-medium text-gray-400 dark:text-zinc-500">{text}</span>
    </div>
  );
}
...
export default function SortableSelectedList({
  ...
  const workingItems = useMemo<WorkingItem[]>(() => {
    ...
    const ranked: WorkingItem[] = items
  ...
  return (
    <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
      <div className="flex flex-col gap-1.5">
        {rankCutoff === 0 && (
          <StaticEmptyZone
            text={t(variant === 'values' ? 'sortableDragValuesHere' : 'sortableDragSkillsHere')}
          />
        )}

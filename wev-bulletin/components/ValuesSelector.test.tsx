...
describe('ValuesSelector', () => {
  ...
  describe('value selection', () => {
    it('calls onToggle when a value row is clicked', async () => {
      ...
      render(<ValuesSelector {...baseProps} selectedValues={[]} onToggle={handleToggle} />);
    ...
    it('shows checkbox checked for selected values', async () => {
      ...
      render(
        <ValuesSelector {...baseProps} selectedValues={[selectedValue.id]} onToggle={() => {}} />,
      );
      ...
      const row = screen.getAllByRole('option').find((el) => el.textContent?.includes(label));
  ...
  describe('localization', () => {
    it('renders French category labels in modal', async () => {
      const user = userEvent.setup();
      renderWithLocale(<ValuesSelector {...baseProps} selectedValues={[]} locale="fr" />, 'fr');
    ...
    it('shows Done control with French label', async () => {
      ...
      expect(screen.getByRole('button', { name: frMessages.profile.valuesDone })).toBeVisible();
    });
  });
});

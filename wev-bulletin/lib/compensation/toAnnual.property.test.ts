...
describe('Property 3: Null Propagation', () => {
  ...
  describe('null amount → null result', () => {
    it.each(ALL_UNITS.flatMap((unit) => SAMPLE_HOURS.map((h) => ({ unit, h }))))(
      'toAnnual(null, "$unit", $h) === null',
      ({ unit, h }) => {
  ...
  describe('null unit → null result', () => {
    it.each(SAMPLE_AMOUNTS.flatMap((amount) => SAMPLE_HOURS.map((h) => ({ amount, h }))))(
      'toAnnual($amount, null, $h) === null',
      ({ amount, h }) => {
...
describe('Property 2: Annualization Monotonicity', () => {
  ...
  it.each(
    ALL_UNITS.flatMap((unit) =>
      positiveAmounts.flatMap((amount) => SAMPLE_HOURS.map((h) => ({ unit, amount, h }))),
    ),

...
for (const explicit of explicitStrings) {
    ...
    if (dsnPasswordMatch) {
      ...
      const uriPasswordMatch = url.match(/(postgres(?:ql)?:\/\/[^:@/]+):([^:@/]+)(@)/i);
      if (uriPasswordMatch) {
        password = uriPasswordMatch[2];
        try {
          password = decodeURIComponent(password);
        } catch { }
        url = url.replace(uriPasswordMatch[0], uriPasswordMatch[1] + uriPasswordMatch[3]);
      }
    }

    const hasCreds = url.match(/(?:^|\s)password\s*=/i) || (url.includes(":") && url.includes("@"));

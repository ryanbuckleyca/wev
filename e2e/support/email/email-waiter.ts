...
export class EmailWaiter {
  ...
  async waitForLink(
    ...
    const finalRecovered = await this.searchRecentEmails(inboxId, linkHint, since);
  ...
  private async waitForNewEmail(
    ...
    since: Date,
    remainingMs: number
  ): Promise<{ body?: string | null; bodyExcerpt?: string | null; subject?: string | null }> {
  ...
  private extractLinkFromEmail(
    email: { body?: string | null; bodyExcerpt?: string | null; subject?: string | null },
    linkHint: string
  ): string | null {
   ...
   */
  private async createTimeoutError(inboxId: string, since: Date): Promise<Error> {
   ...
   */
  private async checkForQuotaExceeded(inboxId: string, since: Date): Promise<Error | null> {
    try {
      ...
      if (missedEmails.length > 0) {
        const subjects = missedEmails
          .map(email => `"${email.subject ?? '(no subject)'}" at ${email.createdAt}`)
          .join(', ');
   ...
   */
  private logWaitStart(inboxId: string, linkHint: string, since: Date, timeoutMs: number): void {

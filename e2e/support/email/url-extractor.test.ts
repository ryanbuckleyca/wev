...
describe('EmailUrlExtractor', () => {
  ...
  describe('extractMatchingUrl', () => {
    ...
    it('handles quoted-printable soft line breaks', () => {
      const content = 'https://example.com/very-long-url?token=abc=\n123&redirect=home';
      const result = extractor.extractMatchingUrl(content, 'token');

      expect(result).toBe('https://example.com/very-long-url?token=abc123&redirect=home');
    });
    ...
    it('returns longest URL when multiple matches and no preference', () => {
      ...
      expect(result).toBe('https://example.com/much-longer-url-with-params?foo=bar&baz=qux');
    });

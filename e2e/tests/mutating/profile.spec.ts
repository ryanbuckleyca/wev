...
test.describe('Profile editing flow @auth-email', () => {
  ...
  test('enforces skills/values limits and persists ordering', async ({ authPage, page }) => {
    ...
    await test.step('Remove extra value, reorder, and persist after reload', async () => {
      ...
      await expect(page.getByLabel(/^bio$/i)).toHaveValue('This is an automated profile used for E2E tests.');

      const skillsOrderAfterReload = await skillsContainer
        .getByRole('button', { name: /^remove /i })
        .evaluateAll((els) => els.map((el) => el.getAttribute('aria-label')));
      expect(skillsOrderAfterReload[0]).toBeTruthy();

      const valuesOrderAfterReload = await valuesContainer
        .getByRole('button', { name: /^remove /i })
        .evaluateAll((els) => els.map((el) => el.getAttribute('aria-label')));
      expect(valuesOrderAfterReload[0]).toBeTruthy();
    });

...
test.describe("Job board", () => {
  test("loads the English job board with seeded jobs", async ({
    jobBoardPage,
    expectations,
  }) => {
  ...
  test("switches locales without losing repeated query params", async ({
    jobBoardPage,
  }) => {
  ...
  test("search narrows the list to a matching seeded job", async ({
    jobBoardPage,
    expectations,
  }) => {
    await loadEnglishJobBoard(jobBoardPage);
  ...
  test("shows the empty state when search removes every job", async ({
    jobBoardPage,
    expectations,
  }) => {
    await loadEnglishJobBoard(jobBoardPage);
  ...
  test("paginates through the seeded dataset", async ({
    jobBoardPage,
    expectations,
  }) => {
    await loadEnglishJobBoard(jobBoardPage);
  ...
  test("filters to jobs posted within one week", async ({
    jobBoardPage,
    expectations,
  }) => {
    await loadEnglishJobBoard(jobBoardPage);
    ...
    await expectVisibleResults(
      jobBoardPage,
      expectations.oneWeekCount,
      expectations,
    );
  });

  for (const key of ["remote", "hybrid", "office"] as const) {
    test(`filters by work type: ${FILTER_LABELS.workType[key]}`, async ({
      jobBoardPage,
      expectations,
    }) => {
      await loadEnglishJobBoard(jobBoardPage);
  ...
  for (const key of ["csi", "goodwork", "centraide"] as const) {
    test(`filters by source: ${FILTER_LABELS.source[key]}`, async ({
      jobBoardPage,
      expectations,
    }) => {
      await loadEnglishJobBoard(jobBoardPage);
  ...
  for (const key of ["fullTime", "contract"] as const) {
    test(`filters by employment type: ${FILTER_LABELS.employmentType[key]}`, async ({
      jobBoardPage,
      expectations,
    }) => {
      await loadEnglishJobBoard(jobBoardPage);
  ...
  for (const key of ["on", "qc"] as const) {
    test(`filters by province: ${FILTER_LABELS.province[key]}`, async ({
      jobBoardPage,
      expectations,
    }) => {
      await loadEnglishJobBoard(jobBoardPage);
  ...
  test("filters by municipality", async ({ jobBoardPage, expectations }) => {
    ...
    await jobBoardPage.toggleFilterCheckbox(
      "municipality",
      FILTER_LABELS.municipality.toronto,
    );
